...
def data_modeling(df): 
    ...
    df.rename(columns={'RETAIL SALES': 'RETAIL_SALES',
    ...
    date_df["DATE_ID"] += 1
    fact_table = df.merge(supplier_df, on = 'SUPPLIER').merge(item_df, left_on="ITEM CODE", right_on="ITEM_CODE").merge(date_df, on= ["YEAR", "MONTH"])[['ITEM_CODE', 'SUPPLIER_ID', 'DATE_ID', 'RETAIL_SALES', 'RETAIL_TRANSFERS', 'WAREHOUSE_SALES']]
